...
const DeviceInfoPage = ({ device, onBack }) => {
    ...
    // Fetch data based on the device prop
    const fetchData = useCallback(async () => {
        ...
        try {
            // Fetch Patient Info (if patient is assigned)
            // The device object from getAllDevices might contain current_patient name,
            // but for detailed info (age, gender, admission date), we need a separate API call.
            // The new endpoint gets details based on the *current* assignment linked to the device ID.
            const patientData = await getPatientDetailsByDevice(device.id);
            // Check if the endpoint returned patient details or a 'not found' message
            if (patientData && patientData.detail !== 'No patient currently assigned to this device.') {
                setPatientInfo(patientData);
            } else {
                // Handle case where no patient is assigned or details are not found
                setPatientInfo(null); // Or set a default object like { name: 'No Patient', age: 'N/A', ... }
            }


            // Fetch Patient Bed Assignment History (linked to the patient currently assigned to the device)
            const patientBedHist = await getPatientAssignmentHistoryByDevice(device.id);
            setPatientBedHistory(patientBedHist);

            // Fetch Device Bed Assignment History (linked to the specific device ID)
    ...
    // Fetch data when the component mounts or when the device prop changes
    useEffect(() => {
        fetchData();
    }, [fetchData]);
            ...
            <div className="mb-6">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <button
                        ...
                        className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg font-medium hover:bg-gray-300 transition-colors shadow-sm"
                        disabled={loading}
                    >
                ...
                <div className="bg-white rounded-xl shadow-md p-6 mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div className="border border-gray-200 rounded-lg p-4">
                            ...
                            <p className="text-lg font-semibold">{patientInfo.admitted_at ? new Date(patientInfo.admitted_at).toLocaleDateString() : 'N/A'}</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="bg-white rounded-xl shadow-md p-6 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div className="border border-gray-200 rounded-lg p-4">
                        ...
                        <p className="text-lg font-semibold">{device.lastReading ? new Date(device.lastReading).toLocaleString() : 'N/A'}</p>
                    </div>
                    <div className="border border-gray-200 rounded-lg p-4">
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {patientBedHistory.map((entry) => (
                                            <tr key={entry.id}>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{entry.bed.bed_number}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{new Date(entry.start_time).toLocaleString()}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                    {entry.end_time ? new Date(entry.end_time).toLocaleString() : <span className="text-green-600 font-medium">Currently Assigned</span>}
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {deviceBedHistory.map((entry) => (
                                            <tr key={entry.id}>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{entry.device.mac_address}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{entry.user.name}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{entry.bed.bed_number}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{new Date(entry.start_time).toLocaleString()}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
